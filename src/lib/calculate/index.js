...
// we use BigNumber because it's API is nice and we don't have to deal with
// floating point errors in testing
import BigNumber from 'bignumber.js'

// data interpolation
const interpolateIncomeCentile = interpolate(INCOME_CENTILES.map(centile => ({ x: centile.percentage, y: centile.international_dollars })))
export const interpolateIncomeCentileByAmount = amount =>
  BigNumber(interpolateIncomeCentile({y: amount}))
    .decimalPlaces(1)
    .toNumber()

export const interpolateIncomeAmountByCentile = centile =>
  BigNumber(interpolateIncomeCentile({ x: centile}))
    .decimalPlaces(2)
    .toNumber()
...
export const getCurrency = countryCode => currencies(countryCode)[0]
